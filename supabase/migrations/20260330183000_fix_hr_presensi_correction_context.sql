...
begin
  ...
  if v_shift_found and v_shift.scheduled_checkin is not null then
    v_scheduled_checkin := ((p_attendance_date::text || ' ' || v_shift.scheduled_checkin::text || '+07')::timestamptz);
  end if;

  if v_shift_found and v_shift.scheduled_checkout is not null then
  ...
  select
    ...
    case when v_shift_found then coalesce(v_shift.grace_minutes, 0) else 0 end;
end;
$$;
